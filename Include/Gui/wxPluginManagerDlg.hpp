...
private:
	void InitGUI()
	{
		...
		m_sizer->Add(text4, 0, wxALL | wxEXPAND, 1);
		...
		m_sizer->Add(listPlugins, 1, wxALL | wxEXPAND, 5);
...
private:
	...
	void UpdateTotalInfo()
	{
		if(pluginManager.GetHandle())
		{
			...
			int count = 0;
			{
				NArrayWrapper< ::Neurotec::Plugins::NPlugin> currentPlugins = pluginManager.GetPlugins().GetAll();
	...
	void OnPluginManagerChanged()
	{
		...
		if(pluginManager.GetHandle())
		{
			...
			for (int i = 0; i < currentPlugins.GetCount(); i++)
			{
				AddPlugin(currentPlugins[i]);
			}
			if (currentPlugins.GetCount() != 0)
			{
				listPlugins->SetColumnWidth(0, wxLIST_AUTOSIZE);
				listPlugins->SetColumnWidth(1, wxLIST_AUTOSIZE);
			}
