...
#if defined(N_MSVC) && defined(_NOEXCEPT)
	#define N_NOEXCEPT _NOEXCEPT
#elif (defined(N_GCC) || defined(N_CLANG)) && defined(_GLIBCXX_USE_NOEXCEPT)
	#define N_NOEXCEPT _GLIBCXX_USE_NOEXCEPT
#else
	#define N_NOEXCEPT
#endif

#if defined(N_MSVC) && defined(_CONST_DATA)
	#define N_CONST_DATA _CONST_DATA
#elif (defined(N_GCC) || defined(N_CLANG)) && defined(_GLIBCXX_USE_CONSTEXPR)
	#define N_CONST_DATA _GLIBCXX_USE_CONSTEXPR
#else
	#define N_CONST_DATA
#endif
...
#else
#if defined(N_CPP11)
	#define N_THROW(args) noexcept(false)
	#define N_THROW_NONE noexcept
	#define N_THROW_ANY noexcept(false)
#else
...
public:\
	...
	bool operator==(const name & p_value) const\
	{\
		return this->value == p_value.value;\
	}\
	bool operator!=(const name & p_value) const\
	{\
		return this->value != p_value.value;\
...
public:\
	...
	bool operator==(const name & p_value) const\
	{\
		return baseName::operator==(p_value);\
	}\
	bool operator!=(const name & p_value) const\
	{\
		return baseName::operator!=(p_value);\
...
public:\
	...
	name(const struct name##_ & p_value)\
	{\
		memcpy(reinterpret_cast<void *>(this), &p_value, sizeof(p_value));\
	}\
	name & operator=(const struct name##_ & p_value)\
	{\
		memcpy(reinterpret_cast<void *>(this), &p_value, sizeof(p_value));\
		return *this;\
	}
...
#define N_DISPOSABLE_STRUCT_MOVE_CONSTRUCTOR(name)\
	...
	{\
		...
		memset(reinterpret_cast<void *>(&other), 0, sizeof(other));\
	}\
...
public:\
	bool operator==(const name & p_value) const\
	{\
		return name::NativeTypeOf().AreValuesEqual(this, sizeof(p_value), &p_value, sizeof(p_value));\
	}
...
#define N_DEFINE_DISPOSABLE_STRUCT_TYPE_TRAITS(typeNamespace, type) \
	...
	{\
		...
		{\
			...
			static typeNamespace::type FromNative(NativeType value, bool ownsHandle) { typeNamespace::type s; if (ownsHandle) memcpy(reinterpret_cast<void *>(&s), reinterpret_cast<void *>(&value), sizeof(NativeType)); else type##Set(&value, &s); return s; }\
...
public:
		...
		: arNativeValues(NULL), count(0), ownsHandles(true), ownsPtr(true)
	{
		...
		for (; first != last; ++first)
		{
			...
			memset(reinterpret_cast<void *>(&arNativeValues[count]), 0, sizeof(arNativeValues[0]));
	...
	NArrayWrapper(const NArrayWrapper<T> & other)
	{
		...
		if (other.ownsPtr)
		{
			...
			{
				memcpy(reinterpret_cast<void *>(arNativeValues), other.arNativeValues, sizeof(other.arNativeValues[0]) * count);
			}
		}
		else
		{
...
public:
	...
	NInt ToByteArray(NByte * pDstArray, NInt dstLength) const
	{
		NInt length;
...
private:
	NInt NGuidCmp(const NGuid & value) const
	{
		//NOTE: NGuid comparison operators should honor strict weak ordering requirements.
		// One of requirement for strict weak ordering is transitivity
		// * if a > b and b > c then a > c
		// standard return a - b; comparison might violate transitivity during underflow, thus do not use it.

		if (Data1 != value.Data1)
			return Data1 < value.Data1 ? -1 : 1;

		if (Data2 != value.Data2)
			return Data2 < value.Data2 ? -1 : 1;

		if (Data3 != value.Data3)
			return Data3 < value.Data3 ? -1 : 1;

		return memcmp(Data4, value.Data4, sizeof(Data4));
	}

public:
	bool operator>(const NGuid & value) const
	{
		return NGuidCmp(value) > 0;
	}
...
public:
	static NVersionRange Intersect(const NVersionRange & nVersionRange1, const NVersionRange & nVersionRange2)
	{
		return NVersionRange(NVersionRangeIntersect(nVersionRange1.value, nVersionRange2.value));
	}
	...
	bool Contains(const NVersion & nVersion) const
	{
		return NVersionRangeContains(this->value, nVersion.GetValue()) != 0;
	}

	bool Contains(const NVersionRange & nVersionRange) const
	{
		return NVersionRangeContainsRange(this->value, nVersionRange.value) != 0;
	}

	bool IntersectsWith(const NVersionRange & nVersionRange) const
	{
		return NVersionRangeIntersectsWith(this->value, nVersionRange.value) != 0;
	}

	NVersionRange Intersect(const NVersionRange & nVersionRange) const
	{
