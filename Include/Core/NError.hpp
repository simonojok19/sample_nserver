...
public:
	...
	static void Report(NResult errorCode, const NError & error)
	{
...
inline void N_NO_RETURN NThrowException(NResult code = N_E_FAILED, const NStringWrapper & message = NString(), const NStringWrapper & paramName = NString()) { ::Neurotec::NError err(code, message, paramName); ::Neurotec::NError::SetLast(err); throw err; }
