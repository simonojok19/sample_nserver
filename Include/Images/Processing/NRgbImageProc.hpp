...
public:
		...
		NUInt resultWidth, NUInt resultHeight, NInterpolationMode interpolationMode)
	{
		...
		NCheck(NrgbipScale(srcImage.GetHandle(), srcLeft, srcTop, srcWidth, srcHeight, resultWidth, resultHeight, interpolationMode, &hResultImage));
	...
	{
		HNImage hResultImage;
		NCheck(NrgbipScale(srcImage.GetHandle(), 0, 0, srcImage.GetWidth(), srcImage.GetHeight(), resultWidth, resultHeight, interpolationMode, &hResultImage));
		return NObject::FromHandle<NImage>(hResultImage);
	}
	...
	{
		NCheck(NrgbipScaleDst(srcImage.GetHandle(), srcLeft, srcTop, srcWidth, srcHeight, dstImage.GetHandle(), dstLeft, dstTop, dstWidth, dstHeight, interpolationMode));
	}
	...
	{
		NCheck(NrgbipScaleDst(srcImage.GetHandle(), 0, 0, srcImage.GetWidth(), srcImage.GetHeight(), dstImage.GetHandle(), dstLeft, dstTop, dstWidth, dstHeight, interpolationMode));
	}
