...
public:
	...
	static ANRecordType GetType20()
	{
		return GetObject<ANRecordType>(ANRecordTypeGetType20Ex, true);
	}

	static ANRecordType GetType21()
	{
		return GetObject<ANRecordType>(ANRecordTypeGetType21Ex, true);
	}
	...
	NString GetFieldXmlElementN(const NVersion & version, NInt fieldNumber) const
	{
		HNString hValue;
		NCheck(ANRecordTypeGetFieldXmlElementN(GetHandle(), version.GetValue(), fieldNumber, &hValue));
		return NString(hValue, true);
	}
