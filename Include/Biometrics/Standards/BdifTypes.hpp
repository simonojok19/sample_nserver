...
N_DEFINE_ENUM_TYPE_TRAITS(Neurotec::Biometrics::Standards, BdifIrisImageFormat)
N_DEFINE_ENUM_TYPE_TRAITS(Neurotec::Biometrics::Standards, BdifEncodingType)
N_DEFINE_ENUM_TYPE_TRAITS(Neurotec::Biometrics::Standards, BdifConformanceTestResult)
...
public:
	...
	BdifFaceFeaturePoint(BdifFaceFeaturePointType type, NByte code, NUShort x, NUShort y, NUShort z)
	{
		Type = type;
		Code = code;
		X = x;
...
class BdifConformanceTest : public BdifConformanceTest_
{
	N_DECLARE_EQUATABLE_DISPOSABLE_STRUCT_CLASS(BdifConformanceTest)

public:
	BdifConformanceTest(const NStringWrapper & test, NInt fieldIndex, const NStringWrapper & fieldName, BdifConformanceTestLevel level, 
						BdifConformanceTestResult result, const NStringWrapper & resultMsg)
	{
		NCheck(BdifConformanceTestCreateN(test.GetHandle(), fieldIndex, fieldName.GetHandle(), level, result, resultMsg.GetHandle(), this));
	}

	NString GetTest() const
	{
		return NString(hTest, false);
	}
	void SetTest(const NStringWrapper & value)
	{
		NCheck(NStringSet(value.GetHandle(), &hTest));
	}

	NString GetFieldName() const
	{
		return NString(hFieldName, false);
	}
	void SetFieldName(const NStringWrapper & value)
	{
		NCheck(NStringSet(value.GetHandle(), &hFieldName));
	}

	NString GetResultMessage() const
	{
		return NString(hResultMessage, false);
	}
	void SetResultMessage(const NStringWrapper & value)
	{
		NCheck(NStringSet(value.GetHandle(), &hResultMessage));
	}
};
...
public:
	...
	static NType BdifEncodingTypeNativeTypeOf()
	{
		return NObject::GetObject<NType>(N_TYPE_OF(BdifEncodingType), true);
	}

	static NType BdifConformanceTestLevelNativeTypeOf()
	{
		return NObject::GetObject<NType>(N_TYPE_OF(BdifConformanceTestLevel), true);
	}

	static NType BdifConformanceTestResultNativeTypeOf()
	{
		return NObject::GetObject<NType>(N_TYPE_OF(BdifConformanceTestResult), true);
	}
...
N_DEFINE_DISPOSABLE_STRUCT_TYPE_TRAITS(Neurotec::Biometrics::Standards, BdifConformanceTest);
