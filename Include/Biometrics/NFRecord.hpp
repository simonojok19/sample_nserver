...
#undef NFR_MAX_QUALITY_BLOCK_COUNT
...
const NInt NFR_MAX_QUALITY_BLOCK_COUNT = 255;
...
public:
	...
	class QualityBlockCollection : public ::Neurotec::Collections::NCollectionWithAllOutBase<NBiometricQualityBlock, NFRecord,
		NFRecordGetQualityBlockCount, NFRecordGetQualityBlock, NFRecordGetQualityBlocks>
	{
		QualityBlockCollection(const NFRecord & owner)
		{
			SetOwner(owner);
		}
	...
	public:
		...
		{
			NInt value;
			NCheck(NFRecordGetQualityBlockCapacity(this->GetOwnerHandle(), &value));
			return value;
		}

		void SetCapacity(NInt value)
		{
			NCheck(NFRecordSetQualityBlockCapacity(this->GetOwnerHandle(), value));
		}

		void Set(NInt index, const NBiometricQualityBlock & value)
		{
			NCheck(NFRecordSetQualityBlock(this->GetOwnerHandle(), index, &value));
		}

		NInt Add(const NBiometricQualityBlock & value)
		{
			NInt index;
			NCheck(NFRecordAddQualityBlock(this->GetOwnerHandle(), &value, &index));
			return index;
		}

		void Insert(NInt index, const NBiometricQualityBlock & value)
		{
			NCheck(NFRecordInsertQualityBlock(this->GetOwnerHandle(), index, &value));
		}

		void RemoveAt(NInt index)
		{
			NCheck(NFRecordRemoveQualityBlockAt(this->GetOwnerHandle(), index));
		}

		void Clear()
		{
			NCheck(NFRecordClearQualityBlocks(this->GetOwnerHandle()));
		}
	};
