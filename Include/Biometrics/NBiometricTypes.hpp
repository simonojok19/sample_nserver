...
class NBiometricQualityBlock : public NBiometricQualityBlock_
{
	N_DECLARE_EQUATABLE_STRUCT_CLASS(NBiometricQualityBlock)

public:
	NBiometricQualityBlock(NByte qualityScore, NUShort qualityAlgorithmVendorId, NUShort qualityAlgorithmId)
	{
	...
	{
		HNString hValue;
		NCheck(NBiometricQualityBlockToStringN(this, format.GetHandle(), &hValue));
		return NString(hValue, true);
	}
};
...
N_DEFINE_STRUCT_TYPE_TRAITS(Neurotec::Biometrics, NBiometricQualityBlock);
